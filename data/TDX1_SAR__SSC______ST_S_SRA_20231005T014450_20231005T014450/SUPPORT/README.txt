TMSP V4.12 Level 1b Product
---------------------------

Staring Spotlight Azimuth Spectrum:

Narrow gaps observed in the azimuth spectrum of Staring Spotlight images at early and late azimuth due 
to variations of the squint angle step size in azimuth beam steering are now largely mitigated by an adjusted 
azimuth subaperture forming and subaperture processing bandwidth selection.

Handling of potentially missing GPS time synchronization:

Since early 2022 the GPS Mosaic receivers may suffer from GPS signal loss for orbit tracks 
passing Eastern Europe. This may happen in cases of active or passive interference with/suppression 
of GPS-signals by on-ground entities or other disturbances in the GPS satellite data reception. 
This leads to missing synchronization of GPS time and on board time.
The instrument indicates this specific state of a possible timing offset with 
an error flag and there is no further information on the amount of such a 
possible timing deviation. 
In this case, the level 1B product will contain a disclaimer file named 
GEOLOCATION_AND_TIMING_PROBLEM.txt
located in the SUPPORT directory of the product.
It is NOT possible to estimate or correct this 
SAR data timing deviation since there is no external reference or measurement 
of the actual offset. A re-processing of this product will NOT mitigate this 
possible error which is embedded in the instrument raw data. 
The orbit data timing is NOT affected since it is derived from independent 
GPS receivers.
The user is advised to check a possible image location offset of this specific 
product with ground control points and/or reference image data. 
In case of interferometric processing, this product is not suited as an 
InSAR-primary image and the azimuth location mismatch may require wider search 
windows for image co-registration and yield phase/spectral mismatches.

TMSP V4.12 Release Remark
-------------------------

Estimation of USO frequency from extracted house keeping temperature measurements:
The TMSP derives the USO frequency from XHK temperature data in case a computation rule is provided in the IOCS-AUX product.
