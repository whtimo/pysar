...
import time
import argparse
from pysar.psi import ps_parameter_estimation_periodogram
...
if __name__ == "__main__":
    ...
    #ref_point = 0
    ref_point = ps_parameter_estimation_periodogram.find_matching_point_index(args.ref_point, args.ps_psc, args.ps)
    #print("Reading the network") # Adding some comments because it is a long process
    #ps_network = PSNetwork(dates, "/path/to/xml/files")
    ps_info = ps_parameter_estimation_periodogram.PSInfo(dates, args.interferogram_dir,  args.ps_phases)

    parameter_estimator = ps_parameter_estimation_periodogram.ParameterEstimator(ps_info)
    ...
    ps_parameter_estimation_periodogram.save_point_data_to_csv(args.ps_phases, args.output_path, params)
