...
def subpixel_shift(master: np.ndarray, slave: np.ndarray, upsample_factor=16):
    """
    ...
    m, n = master.shape
    upsampled_m = fftpack.fftshift(fftpack.fft2(master, (m*upsample_factor, n*upsample_factor)))
    ...
    padded_shape = [s + m for s, m in zip(upsampled_slave.shape, upsampled_master.shape)]
    f_master = fftpack.fft2(upsampled_master, padded_shape)
    f_slave = fftpack.fft2(upsampled_slave, padded_shape)
...
def parameter_estimation(shifts, degree:int = 2):
    ...
    x = shifts['masterX'].to_numpy()
    y = shifts['masterY'].to_numpy()
    dx = shifts['shiftX'].to_numpy()
    dy = shifts['shiftY'].to_numpy()

    print(f'Mean: {np.mean(dx)}, {np.mean(dy)}')
    ...
    # Fit models
    ransac_pipeline_dx.fit(np.column_stack([x, y]), dx)
    ransac_pipeline_dy.fit(np.column_stack([x, y]), dy)
